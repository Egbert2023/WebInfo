...
var eaCookiesDirektive = function($rootScope, $cookies, $compile) {
    return {
        ...
        controller: function($scope) {
            ...
            // Develop environment - "domain" : "localhost",
            // Live environment - "domain" : "aleksander.de",
            var today = new Date();
            var todayAdd = new Date();
            todayAdd.setDate(today.getDate()+10);
            $scope.CookieProviderOptions = {
                "path" : "/",
                "secure" : true,
                "expires" : todayAdd,
...
//                }    
            ...
            let saveCookies = function() {
                for(let i=0; i<$scope.cookies.length; i++) {
                    $cookies.put("WebInfo_Cookie_" + $scope.cookies[i].name, 
                        $scope.cookies[i].value.toString(),
                        $scope.CookieProviderOptions);
                }
            };
            ...
            $scope.cookiePut = function(key, o, opt) { 
                if(opt) {
                    $cookies.put(key, o.toString(), opt);
                } else {
                    $cookies.put(key, o.toString());
                }
                return false;
            };
            ...
            $scope.cookieRemove = function(key, opt) { 
